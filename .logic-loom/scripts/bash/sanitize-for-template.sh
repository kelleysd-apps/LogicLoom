...
See [.logic-loom/config/architecture.conf](.logic-loom/config/architecture.conf) for complete configuration.
...
- `.logic-loom/config/architecture.conf` - Architecture configuration
...
cat .logic-loom/memory/constitution.md

# Follow update checklist if modifying
cat .logic-loom/memory/constitution_update_checklist.md
```
...
bash .logic-loom/scripts/bash/sync-plugin-commands.sh list
