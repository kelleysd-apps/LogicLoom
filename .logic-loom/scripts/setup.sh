...
chmod +x .logic-loom/scripts/bash/*.sh 2>/dev/null || true
...
if [ -x ".logic-loom/scripts/bash/sync-plugin-commands.sh" ]; then
    .logic-loom/scripts/bash/sync-plugin-commands.sh sync 2>/dev/null || true
...
echo -e "  Edit: ${GREEN}.logic-loom/memory/constitution.md${NC}"
...
echo -e "  • ${GREEN}.logic-loom/memory/constitution.md${NC} - Development principles"
