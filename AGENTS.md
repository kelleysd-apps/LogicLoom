...
|---------|------|---------|
| 6.6.2 | 2026-08-31 | **The npm package ships a README.** 6.6.0 and 6.6.1 both published without one, so the registry showed "This package does not have a README" as the front page. Invisible to every existing check because `files` does not list README.md — npm includes it unconditionally — so nothing that reads that array could see it missing. Contract test now pins its existence, that it names `npx logicloom init`, that it states the no-`--force` refusal, and that `files` still omits it. Content is decision-shaped for an adopter: plan/apply split, what each `--only` target installs, why `hooks` is excluded from `all`, the four code-enforced refusals, the receipt's uninstall, and the runtime requirements the plan surfaces before an install |
| 6.6.1 | 2026-08-29 | **First release published by CI.** v6.6.0 claimed the `logicloom` npm name with a manual publish (no name reservation exists, and npm's trusted-publisher page needs the package to exist first), so its tarball was built locally. **`npm publish` made idempotent** — publishing over an existing version fails E403 and the step had no guard, which made a re-run, and the first release of any hand-claimed name, red by construction; it now checks the registry and exits 0 with a notice. **Payload restored to CI's build**: v6.6.0 shipped 276 payload files against the workflow's 278, the local build having dropped `plugins/loom-memory/{recall,working}/.gitkeep` via a `git ls-files` that honoured the tag tree's own .gitignore — self-healing at runtime, corrected here (LOOM-0045). Docs and the docs-sync contract test move to the published state: `npx logicloom init .` is presented as runnable |
| 6.6.0 | 2026-08-28 | **`npx logicloom init` — the adopt package**: install LogicLoom into an existing repository, or start a new one. A read-only PLAN the adopter reviews and answers, then an `--apply` that writes only the targets they named — never deleting, truncating, overwriting a file it did not create (`copyFileNew` opens `'wx'`), moving source, or running mutating git; no `--force`. Every write recorded in `.logicloom-adopt-receipt.json`, whose `uninstall` object is the reversal procedure. `--agent-guide` + `--json` make it drivable by a coding agent. **`.brain/`**: a project-scoped knowledge layer with one resolver for where memory lives, so retrieval follows the write target. **Promotion lifecycle** `/promote-dev` → `/promote-staging` → `/promote-prod`, and a draft GitHub Release on tag with a command that asserts it happened. _(the correctness half:)_ **the governance floor failed OPEN** on a machine without `jq` and `python3` — two guards returned ALLOW where they deny everywhere else, so Principle VI and the rule-protection hook silently lapsed on an ordinary slim container; both now carry the raw-text rung `git-safety-gate.sh` already had. **The uninstall could destroy the adopter's own files** — its delete list named the two merge targets and the sidecar a later step still had to read, with directories flat in the same list; now split files/`dirsIfEmpty`, merges excluded, and every created file carries a `sha256` so deletion is conditional on the file still being ours. **Nested installs** blocked, the **adopter's execution environment** probed and reported in the plan, and `engines.node` corrected from a publish-time CI floor to the one CI proves |
