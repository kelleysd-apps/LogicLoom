...
**`npx logicloom` resolves** — the package is published (`logicloom@6.6.0`,
first published 2026-08-29), so `npx logicloom init .` works in any directory.
You can also run it from a LogicLoom
...
**Jury-on-demand**: picks 1-3 LLM judges based on query type. Pass `--judges all` for the full 3-judge tribunal (Claude + OpenAI + Gemini) on high-stakes questions. Outputs land in `features/<feature-name>/research/`.
