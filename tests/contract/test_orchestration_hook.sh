...
# Operations-log isolation: the hooks this suite drives source policy.sh /
# logging.sh, which otherwise append to the shared
# .logic-loom/logs/operations/ file. LOOM_LOG_DIR redirects that (same idiom as
# LOOM_CHECKPOINT_DIR in .logic-loom/tests/test-git-safety.sh), exported so the
# hook subprocesses inherit it.
