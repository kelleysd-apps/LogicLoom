...
assert "the plugin-bridge wrappers are detected as generated" \
  "grep -q '^\.claude/commands/swarm\.md$' <<< \"\$SHIPPED\""
...
assert "VISION.md stubs from the project-vision template" \
  "grep -qF 'stub: VISION.md :: .logic-loom/templates/project-vision-template.md' <<< \"\$STUB_LINES\""
assert "backlog.md stubs from the project-backlog template" \
  "grep -qF 'stub: .logic-loom/memory/backlog.md :: .logic-loom/templates/project-backlog-template.md' <<< \"\$STUB_LINES\""
...
if [ -f "$STRIPPER" ]; then
  ...
  assert "missing template -> names the template in the error" \
    "grep -q 'stub template not found' <<< \"\$OUT\""
