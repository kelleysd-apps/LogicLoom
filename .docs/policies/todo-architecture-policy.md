...
Both of those paths are **per-feature**. Work that is not a feature — governance, hooks, tests, CI, release tooling, policy and docs — lives one level above them, at **Level 0** (below), which is **two files, not one**: `.logic-loom/memory/todos.md` (active work) and `.logic-loom/memory/backlog.md` (deferred work).
...
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │        REPO-LEVEL TODOS + BACKLOG  (Level 0)             │   │
│  │   (.logic-loom/memory/todos.md   — active, NOW)          │   │
│  │   (.logic-loom/memory/backlog.md — deferred, LATER)      │   │
│  │                                                          │   │
│  │  • Cross-cutting work that is NOT a feature             │   │
│  │  • Governance, hooks, tests, CI, release, policy, docs  │   │
│  │  • ONE grammar, ONE id space, ONE parser across both    │   │
│  │  • Stable minted ids (LOOM-NNNN) — an id NEVER changes  │   │
│  │    when an item is promoted or deferred between them    │   │
...
### Level 0: Repo Todos + Backlog (`.logic-loom/memory/todos.md`, `.logic-loom/memory/backlog.md`)
...
**Level 0 is TWO FILES, one stream.** `todos.md` holds what is being addressed **now** — in progress, next up, or waiting only on a decision already asked for. `backlog.md` holds what to bring up **later** — explicitly deferred, a "decide later", or an analysis with no consumer yet. The split exists because one mixed list is unreadable: the item you must act on today sits between two you decided six weeks ago not to act on at all.

What is **shared**, and is not negotiable:

- **One grammar.** Specified once, in `backlog.md`'s header. `todos.md` points at it and restates nothing — two copies of a normative grammar disagree the first time one is edited.
- **One id space.** Ids are unique **across both files**, because `blocked_on:` references cross the streams — a deferred item is routinely blocked on an active decision, and vice versa. The counter is **derived, never stored**: next id = (highest id present in either file) + 1, computed by `lint-backlog.sh --next-id`. A stored counter would live in one file and be silently wrong the moment an item was appended to the other.
- **One parser and one linter.** `build-backlog-index.sh` reads both (levels `todo` and `backlog`); `lint-backlog.sh` lints both and reports a cross-file `duplicate-id`. A colliding mint is fatal to the collector — non-zero exit, no index written.

**Promotion** is a cut-and-paste of the item's whole block between the two files, and **the id never changes** — not on promote, not on defer, not on a title edit made while moving it. An item's stream is a *property* of the item, not part of its identity, which is why `level` is derived by the collector from the file path rather than written down by anyone. A `done` item stays where it was completed, which is `todos.md`. The full rule, including what may and may not be edited during a move, is § *Promotion* in `todos.md`.
...
- Generated by the `unified-specification` skill (tasks phase) via the `/specification` command
- Persists in version control
...
**When to Update**:
- After the `/specification` tasks phase generates the initial list
...
### /specification Command Integration

The `/specification` command generates the project-level SSOT:

```bash
# Full waterfall: spec -> plan -> tasks
/specification "feature description"
# OR: the tasks phase alone, when plan artifacts already exist
...
```
0. Read .logic-loom/memory/todos.md (active) and .logic-loom/memory/backlog.md (deferred) for cross-cutting harness work
...
- **Repo todos (active, Level 0)**: `.logic-loom/memory/todos.md` — also § *Promotion* and the shared-id-space rule
- **Repo backlog (deferred, Level 0) + item grammar (normative for both)**: `.logic-loom/memory/backlog.md`
...
| Version | Change |
|---|---|
| 1.2.0 | **Split Level 0 into two files**: `.logic-loom/memory/todos.md` (active work — in progress, next up, or waiting only on a decision already asked for) and `.logic-loom/memory/backlog.md` (deferred work — "decide later", no consumer yet). One mixed stream was unreadable. What did NOT split: the grammar (still specified once, in `backlog.md`, and normative for both), the parser, the linter, and the **id space** — ids stay unique across both files because `blocked_on:` references cross the streams, and the counter became **derived** (`lint-backlog.sh --next-id`) rather than stored in a footer that only one of the two files could hold. Promotion between the streams is a cut-and-paste that **never changes an item's id**. Collector levels `todo` and `backlog`; the dashboard renders them as separate top-level sections, todos first. |
