...
## Path Conventions (LogicLoom + Legacy SDD)

This policy supports two coexisting workflows per the v3 supplementary principle:

- **LogicLoom (primary)**: project tasks live in `features/<name>/plan.md` (DAG plan)
- **Legacy SDD (preserved)**: project tasks live in `specs/###-feature/tasks.md`

Where this document references `specs/###-feature/tasks.md`, treat it as equivalent to `features/<name>/plan.md` for LogicLoom features. Session-level (Task tools) and agent-level (`.docs/agents/*/decisions/tasks/`) layers are identical across both.
...
**LogicLoom (primary)**: `features/<name>/plan.md`
...
**Legacy SDD**: `specs/###-feature/tasks.md`
- Generated by `sdd-tasks` skill via `/tasks` or `/specification` command (legacy waterfall)
- Persists in version control
...
- Marked with parallel execution indicators [P] (legacy) or independent edges (DAG)
