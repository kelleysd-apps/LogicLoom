...
Hook-level enforcement is a deliberate design decision: a prompt-level reminder is bypassable by any worker that ignores
...
The workaround is local to the hook. The definitive fix is to add `: "${DEBUG:=0}"` (or
equivalent) at the top of `logging.sh`. Until then, any other strict-mode caller of logging.sh will hit the
same issue and need the same workaround.

## 8. Sidecar file

`worktree-port-namespace.sh` (a separate hook) writes a sidecar
