...
These commands were **REMOVED** — no command file backs them and invoking one
does nothing. They are listed only so an agent carrying stale knowledge of the
old three-phase workflow is redirected. Use `/specification`.

| Removed (does not exist) | Use Instead |
|------------|-------------|
