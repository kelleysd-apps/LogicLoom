...
Parse `<feature-name>` (positional or `--feature`). Verify `features/<feature-name>/sprints/` exists, then read sprint outputs, the git log since branch creation, the plan-vs-actual DAG diff, and `plan-review.md` if present. Synthesize wins / misses / action-items and write `features/<feature-name>/retro.md` (overwrite). Then write the action-items section to `$(bash .logic-loom/scripts/bash/resolve-memory-backend.sh --ensure)/retro_<feature>_<YYYY-MM-DD>.md` — resolve the directory, never hardcode it; the backend (`repo` default / `project`) is set in `.logic-loom/config/memory-backend.conf`. The default is `repo` (`.brain/memory/`); resolution is a pure function of env + config and never probes the filesystem. A prior store left behind at `$HOME/.claude/projects/<slug>/memory/` is reported by the preflight advisory, not by this command. Skip if that file already exists for the same day.
