...
**SKILL ACTIVATION**: Read and execute `plugins/loom-orchestrator/skills/swarm-implement/SKILL.md`. Reads `features/<active-feature>/plan.md` (DAG format per `.logic-loom/templates/plan-template.md`), validates file-ownership (no two tasks owning same path), topologically sorts within the sprint, dispatches workers with `LOOM_ACTIVE_FEATURE` / `LOOM_ACTIVE_TASK` env vars set so the freeze-write-scope hook gates writes. Hard gate: refuses to proceed unless `features/<active-feature>/plan-review.md` contains `Overall verdict: go`.
